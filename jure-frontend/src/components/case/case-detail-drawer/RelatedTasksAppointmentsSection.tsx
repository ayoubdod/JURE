...
import { formatDate, formatTime, useAppTranslation, type Lang } from '@/i18n';
...
function dueTone(
  ...
  const dateText = formatDate(dueIso, lang, { day: 'numeric', month: 'short', year: 'numeric' }) || '—';
  const label = interpolate(dueTemplate, { date: dateText });
  const overdue = days != null && days < 0 && status !== TaskStatus.DONE;
  if (overdue) return { cls: 'text-red-700 dark:text-red-400 font-semibold', overdue: true, label };
  if (days == null) return { cls: 'text-slate-500 dark:text-slate-400', overdue: false, label };
  ...
  return { cls: base, overdue: false, label };
}
    ...
    >
      <div className="flex flex-wrap items-center gap-2 gap-y-1 mb-4">
        ...
        <span className="text-[10px] text-slate-500 dark:text-slate-400 tabular-nums">
          {tf(related.counts, { tasks: taskCount, appointments: apptCount })}
        </span>
      </div>
      ...
      {showCombinedEmpty ? (
        ...
        <div className="space-y-8">
          {showTasks ? (
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[12px] font-semibold text-slate-800 dark:text-slate-200">{t.cases.card.tasks}</span>
            ...
            {tasks.length === 0 ? (
              <div className="rounded-lg border border-slate-200/80 dark:border-slate-800 bg-white/50 dark:bg-slate-900/30 px-3 py-4 text-center">
                <p className="text-[12px] text-slate-500 dark:text-slate-400 mb-3">{d.noTasks}</p>
              ...
              <ul className="space-y-2">
                {tasks.map((task) => {
                  const due = dueTone(task.due_date, task.status, lang, related.due, tf);
                  const assignee = task.assigned_to
                    ? `${task.assigned_to.first_name || ''} ${task.assigned_to.last_name || ''}`.trim() || task.assigned_to.email
                    : '—';
                  return (
                    <li
                      key={task.id}
                    ...
                    >
                      <div className="flex flex-wrap items-start gap-1.5 mb-1.5">
                        {showPriorityPill(task.priority) && (
                      ...
                      <p className="text-[13px] font-semibold text-slate-900 dark:text-white pe-8">{task.title}</p>
                      <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-600 dark:text-slate-400">
                        ...
                        {task.due_date && (
                          <span className={cn('inline-flex items-center gap-1', due.cls)}>
                            ...
                            {due.label}
                            {due.overdue && (
                              <span className="ms-1 rounded bg-red-500/15 px-1 py-0.5 text-[9px] font-bold uppercase text-red-700 dark:text-red-400">
                                {t.tasks.overdue}
                              </span>
                            )}
                          </span>
                      ...
                      <div className="mt-2 flex items-center justify-between gap-2">
                        {task.estimated_hours ? (
                          <span className="inline-flex items-center gap-1 text-[11px] text-slate-500 dark:text-slate-400">
                            ...
                            {tf(t.calendar.panel.hoursEstimated, { hours: task.estimated_hours })}
                          </span>
                        ) : (
                          <span />
                        )}
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[12px] font-semibold text-slate-800 dark:text-slate-200">{t.cases.card.appointments}</span>
                <span className="text-[10px] font-medium rounded-full bg-slate-200/80 dark:bg-slate-800 px-2 py-0.5 tabular-nums text-slate-700 dark:text-slate-300">
                  {appointments.length}
                </span>
              </div>
            ...
            {appointments.length === 0 ? (
              <div className="rounded-lg border border-slate-200/80 dark:border-slate-800 bg-white/50 dark:bg-slate-900/30 px-3 py-4 text-center">
                <p className="text-[12px] text-slate-500 dark:text-slate-400 mb-3">{d.noAppointments}</p>
                ...
                {appointments.map((a) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[13px] font-semibold text-slate-900 dark:text-white pe-8">{a.title}</p>
                      <p className="mt-1 text-[11px] text-slate-600 dark:text-slate-400">
                        <Calendar className="inline h-3 w-3 me-1 opacity-70 align-text-bottom" aria-hidden />
                        {formatDate(a.start_at, lang, { day: 'numeric', month: 'short', year: 'numeric' }) || '—'} ·{' '}
                        {formatTime(a.start_at, lang)} →{' '}
                        {formatTime(a.end_at, lang)}
                      </p>
                      <div className="mt-1.5 flex flex-wrap items-center justify-between gap-2 text-[11px] text-slate-600 dark:text-slate-400">
                        ...
                        <button
                          ...
                          aria-label={related.openAppointment}
                          onClick={(e) => {
                            e.stopPropagation();
