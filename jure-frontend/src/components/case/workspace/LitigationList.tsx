...
const STORAGE_KEY = 'jure.litigation.list.open';
        ...
        <div className="sm:hidden">
          {loading
            ? Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="h-[88px] animate-pulse border-b border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-950"
                />
              ))
            : error || empty}
        </div>
  ...
  return (
    <div className="space-y-3">
      {visibleSections.map((section) => {
        const items = rows.filter((c) => litigationLevelOf(c) === section.key);
              ...
              <>
                <div className="hidden overflow-x-auto sm:block">
                  <table className="w-full min-w-[880px]">
                    ...
                    <tbody>{items.map((c, i) => renderRow(c, i))}</tbody>
                  </table>
                </div>
