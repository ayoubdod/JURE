...
const sheetVariants = cva(
  "fixed z-50 gap-3 bg-background p-4 transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
interface SheetContentProps
  ...
  /** Merged into `SheetOverlay` (e.g. opacity). */
  overlayClassName?: string
  /**
   * When set, portal mounts here. Overlay and panel use `absolute` so they fill only this
   * ancestor (parent should be `relative`, e.g. the cases page column).
   */
...
>(({ side = "right", className, children, overlayClassName, container, ...props }, ref) => (
  <SheetPortal container={container ?? undefined}>
    <SheetOverlay
      className={cn(
        container != null && "!absolute inset-0 z-40",
        overlayClassName
      )}
    />
