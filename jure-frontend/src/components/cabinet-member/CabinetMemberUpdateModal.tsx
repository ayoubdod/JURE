...
import {
  apiUpdateCabinetMember,
  apiUpdateCabinetMemberRole,
...
function valuesFromMember(member: API.CabinetMember): API.CabinetMemberUpdateForm {
  return {
    id: member.id,
...
const CabinetMemberUpdateModal = forwardRef<
  CabinetMemberUpdateModalRef,
  CabinetMemberUpdateModalProps
>(({ onSuccess }, ref) => {
  ...
  const clearLocalPhoto = useCallback(() => {
    setPhotoFile(null);
  ...
  const show = (member: API.CabinetMember) => {
    ...
    mainForm.reset(valuesFromMember(member));
    setIsOpen(true);
  };
  ...
  const handleSubmit = async (data: API.CabinetMemberUpdateForm) => {
    ...
    try {
      ...
      if (photoFile) {
        try {
          const imageRes = await apiUploadCabinetMemberImage(routeId, photoFile);
          merged = { ...merged, ...imageRes.data, id: instance.id };
        } catch {
          // Profile was saved; optional photo must not fail the flow.
        }
      }

      if (roleChanged && data.role) {
        const roleRes = await apiUpdateCabinetMemberRole({ id: routeId, role: data.role });
        merged = { ...merged, ...roleRes.data, id: instance.id };
      }

      setSubmitPhase('success');
      toast({
        title: t.team.modal.updatedTitle,
        description: tf(t.team.modal.updatedDescription, {
  ...
  const onInvalid = () => {
    const order: (keyof API.CabinetMemberUpdateForm)[] = [
      'first_name',
      'last_name',
      'email',
      'phone',
  ...
  const currentRole = mainForm.watch('role') || instance?.role || 'VIEWER';
  ...
  const avatarSrc = photoPreview || (!photoRemoved ? existingImage : null);
  const hasPhoto = Boolean(avatarSrc);

  const fieldError = (name: keyof API.CabinetMemberUpdateForm) =>
          ...
          >
            ...
            >
              <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3.5">
                      ...
                      <button
                        ...
                        disabled={isBusy}
                        aria-label={hasPhoto ? t.team.modal.photoChange : t.team.modal.photoAdd}
                        className={cn(
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                          <button
                            ...
                            onClick={() => photoInputRef.current?.click()}
                            disabled={isBusy}
                            className="rounded-sm text-[13px] font-medium text-[#64499D] hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#64499D] dark:text-[#CFC2FF]"
                          >
                            {hasPhoto ? t.team.modal.photoChange : t.team.modal.photoAdd}
                          </button>
                          {hasPhoto && (
                            <button
                              type="button"
                              ...
                              disabled={isBusy}
                              className="inline-flex items-center gap-0.5 rounded-sm text-[12px] text-slate-500 hover:text-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500"
                            >
                ...
                <FormSection index="04" title={t.team.modal.sectionPermissions}>
                  <div className="rounded-xl border border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-3 rounded-xl px-3.5 py-2.5 text-start transition-colors duration-200 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#64499D]/30 dark:hover:bg-zinc-900"
                    >
                      <span>
                ...
                <FormSection index="05" title={t.team.modal.sectionStatus}>
            ...
            <DialogFooter className="shrink-0 flex-row items-center justify-end gap-2.5 space-x-0 border-t border-slate-200 bg-white px-6 py-3.5 dark:border-zinc-800 dark:bg-zinc-950 sm:flex-row sm:space-x-0 md:px-7">
              ...
              <Button
                type="submit"
                disabled={isBusy}
                className="h-10 min-w-[180px] bg-[#64499D] px-4 text-white hover:bg-[#4D3680]"
              >
                {submitPhase === 'loading' ? (
                  <>
