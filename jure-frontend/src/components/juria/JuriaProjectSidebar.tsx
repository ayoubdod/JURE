...
function isEmptySimpleChat(p: JuriaProject) {
  return Boolean(p.is_simple) && (p.message_count ?? 0) === 0;
}
...
export function JuriaProjectSidebar({
  ...
  const listed = useMemo(() => {
    ...
    return projects.filter((p) => {
      if (isEmptySimpleChat(p)) return false;
      if (!q) return true;
      return p.name.toLowerCase().includes(q) || (p.description || '').toLowerCase().includes(q);
    });
  }, [projects, debounced]);

  const favorites = useMemo(() => listed.filter((p) => p.is_favorite), [listed]);
  const nonFavorites = useMemo(() => listed.filter((p) => !p.is_favorite), [listed]);
...
function ProjectRow({ p, active, onOpen }: { p: JuriaProject; active: boolean; onOpen: (id: string) => void }) {
  ...
  return (
    ...
    >
      <Icon className={cn('mt-0.5 h-3.5 w-3.5 shrink-0', active ? 'text-[#64499D]' : 'text-slate-400')} />
