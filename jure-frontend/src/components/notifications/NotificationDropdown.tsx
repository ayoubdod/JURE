...
export function NotificationDropdown({ phase, onRequestClose }: NotificationDropdownProps) {
  ...
  const groups = useMemo(() => {
    ...
    const dateGroups = groupNotificationsByDate(rest, t.notifications.groups);
    return { pinnedItems, dateGroups };
  }, [filtered, pins, t.notifications.groups]);

  const allRead = unreadCount === 0;
  const lastUpdated = updatedAt.toLocaleTimeString(intlLocale(lang), { hour: '2-digit', minute: '2-digit' });
  ...
  return (
    <>
      <div
        ...
        role="dialog"
        aria-label={t.notifications.activityAria}
      >
          ...
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] text-slate-500">
                {tf(t.notifications.updatedAt, { time: lastUpdated })}
                {wsConnected ? ` · ${t.notifications.live}` : ''}
              </p>
            </div>
        ...
        <div className="max-h-[min(380px,55dvh)] flex-1 overflow-y-auto overscroll-contain">
          {isLoading ? (
            <div className="flex items-center justify-center py-16 text-sm text-slate-500">{t.notifications.loading}</div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
              ...
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{t.notifications.emptyTitle}</p>
              <p className="mt-1 max-w-[220px] text-xs text-slate-500">
                {activeFilter === 'unread' ? t.notifications.emptyUnread : t.notifications.emptyHint}
              </p>
            </div>
          ) : (
            <div>
              {groups.pinnedItems.length > 0 ? (
                <section>
                  <h3 className="sticky top-0 z-[1] bg-slate-50/95 px-4 py-1.5 text-[10px] font-semibold text-slate-500 backdrop-blur-sm dark:bg-slate-900/95 ltr:uppercase ltr:tracking-wider">
                    {t.notifications.pinned}
                  </h3>
